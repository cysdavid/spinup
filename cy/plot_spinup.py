...
for filename in data_files:
    with h5py.File(filename, mode='r') as f:
        ...
        for it in range(0,f['tasks']['DelOmega'].shape[0],strd):
            ...
            uphi_rel_maxes.append(np.max(np.abs(f['tasks']['uphi'][it]-DelOmega_i*ss_data)))

        file_uphi_rel_max = np.max(uphi_rel_maxes)
        file_uphi_max = np.max(np.abs(f['tasks']['uphi'][::strd]))
        file_us_max = np.max(np.abs(f['tasks']['us'][::strd]))
        file_uz_max= np.max(np.abs(f['tasks']['uz'][::strd]))
